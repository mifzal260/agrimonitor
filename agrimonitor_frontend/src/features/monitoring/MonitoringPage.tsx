...
function ActivitySummary({ records, activities, onUpdate, onDelete }: { records: PlantingRecord[]; activities: Activity[]; onUpdate: (activityId: number, form: { activity_type: string; activity_date: string; description: string; cost_amount: string }) => Promise<void>; onDelete: (activityId: number) => Promise<void> }) {
  ...
  const activitiesByRecord = records.map((record) => ({
    record,
    activities: activities.filter((activity) => activity.planting_record_id === record.id),
  ...
  return (
    <div className="rounded-lg border border-field-100 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h3 className="font-semibold">Recent activities by plot</h3>
          <p className="mt-1 text-xs text-slate-500">Aktiviti diasingkan mengikut petak supaya tidak bercampur.</p>
        </div>
        <StatusBadge label={toCurrency(String(totalCost))} tone={totalCost > 0 ? "warning" : "info"} />
      </div>
      {activitiesByRecord.length === 0 ? (
        <p className="mt-3 text-sm text-slate-600">No records yet.</p>
      ) : (
        <div className="mt-4 space-y-4">
          {activitiesByRecord.map(({ record, activities: plotActivities }) => {
            const plotCost = totalActivityCost(plotActivities);
            return (
              <section key={record.id} className="rounded-lg border border-slate-100 bg-field-50 p-3">
                <div className="flex items-center justify-between gap-3">
                  <div className="min-w-0">
                    <h4 className="truncate text-sm font-bold text-slate-950">{record.field_name}</h4>
                    <p className="text-xs text-slate-600">{record.crop.name}</p>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <StatusBadge label={`${plotActivities.length} aktiviti`} tone="info" />
                    <StatusBadge label={toCurrency(String(plotCost))} tone={plotCost > 0 ? "warning" : "success"} />
                  </div>
                </div>
                  ...
                  {plotActivities.slice(0, 5).map((activity) => (
                    <li key={activity.id} className="py-3">
                      {editingId === activity.id ? (
                        <form onSubmit={submitEdit} className="space-y-2 rounded-md bg-white p-3">
